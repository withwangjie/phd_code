...
This module extends :mod:`evaluate_complex_dockq` with the three corrections
...
* This module is intentionally free of exotic dependencies: pure Python +
  ...
  (which pulls in ``torch``/``torch_geometric`` at import time) even though
  it mirrors two of that module's small data tables; see the note on
  :data:`_SYMMETRIC_SWAPS` above.
"""
...
try:
    ...
    raise ImportError(
        ...
        "(pip install gemmi==0.7.5, matching requirements-audit.txt)"
    ) from exc
...
# Canonical side-chain heavy-atom names, mirroring subgraph_to_qubo.py's
# ``_SIDECHAIN_NAMES`` table (duplicated here, not imported, to keep this
# module free of that file's torch/torch_geometric import-time dependency).
...
# scoring zero atoms.
_SIDECHAIN_HEAVY_ATOMS: Dict[str, Tuple[str, ...]] = {
...
# section. Mirrors subgraph_to_qubo.py's ``_SYMMETRIC_SWAPS`` (duplicated,
# not imported -- see the module docstring's dependency note).
_SYMMETRIC_SWAPS: Dict[str, Tuple[Tuple[str, str], ...]] = {
