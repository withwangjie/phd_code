...
|---|---|---|---|
| Antigen-fold holdout | whole layered-isolation components held out from training by a deterministic name-hash fold; scored by the frozen pipeline without refitting | Literature-informed preregistration | Current SAbDab-derived antibody benchmarks test generalization with cluster-level holdouts of the same snapshot, for example epitope-group, antigen-fold and temporal splits, each enforcing cluster-level separation rather than requiring a separate database. The holdout reuses this study's own stricter isolation relation. A separate external VHH set is unobtainable here (PROTOCOL_AMENDMENTS.md A10), and a two-sided cluster sign-flip test needs at least 6 clusters to reach p<0.05 at all (2/2^G), with few-cluster inference unreliable in general [R43]. |
| External VHH antigen eligibility | at least one antigen chain of SAbDab type protein or peptide; ions, glycans and small-molecule ligands neither qualify nor disqualify an entry | Direct literature basis for the criterion; study-specific for this set | SAbDab's `antigen_type` is the deduplicated set of types over an entry's antigen chains, so a combination such as `protein \| sugar` denotes a polypeptide antigen accompanied by another entity, not a disqualifying antigen. Recent SAbDab-derived benchmarks retain a complex when at least one antigen chain is a protein or peptide, and SAbDab defines protein as a polypeptide of >30 residues and peptide as <30 [R33]. SNAC-DB, the training source, likewise curates the protein complex [R34]. The graph encodes only amino-acid residues, so non-polypeptide entities never enter a graph; requiring their absence would make the external set stricter than the training set it validates (PROTOCOL_AMENDMENTS.md A8). |
| Formal structural sources | `snac_db` primary; `sabdab_vhh` auxiliary; duplicate PDBs resolved SNAC > SAbDab > RCSB before quality/outcome inspection; generic `train_rcsb` audit-only | Study-specific task-definition guard | The task is VHH-antigen interface reconstruction. SNAC-DB explicitly curates VHH-antigen complexes [R34], and SAbDab supplies antibody/antigen chain metadata [R33]. A generic protein-complex contact pair is not evidence that either partner is a VHH, so RCSB structures are not admitted to formal VHH training without an independent strict VHH/partner annotation. |
| Complex definition (antigen chains) | `sabdab_vhh`: first author-determined biological assembly (software-determined fallback; entries without assembly annotation excluded); exactly one source-annotated VHH, no L-chain/scFv, protein/peptide antigen metadata, and only annotated antigen chains with any CA/CB within 7.5 A of the VHH paratope. `snac_db`: SNAC-DB assembly-curated complexes as distributed | Direct literature basis | SAbDab assigns antigens by CA/CB proximity (7.5 A) to CDR residues within the author-defined biological assembly [R33]; SNAC-DB extracts curated complexes with biological-assembly logic [R34]. The asymmetric unit alone is not a biological complex: crystal-packing neighbours can appear as false partners and symmetry-generated partners are missing [R35,R36]. |
| Antibody-antigen interface label | Primary cross-partner heavy-atom distance <=4.5 Å; sensitivity labels at 3.5 and 5.0 Å | Literature-supported preregistration | Krawczyk et al. define antibody-specific epitope contacts with a 4.5 Å heavy-atom cutoff [R49]. Other antibody-interface/CAPRI work uses 5 Å [R3,R4], so 5 Å is retained as a permissive sensitivity definition rather than treated as incorrect; 3.5 Å is the preregistered stricter sensitivity. |
...
| Foldseek/TM-score cluster threshold | min_score=0.50 | Direct literature basis for the structural-similarity cutoff | Foldseek supplies the structure-search method [R5]. Independently, [R27] shows a rapid same-fold/topology transition around TM-score=0.5 and proposes 0.5 as a rough quantitative fold/topology cutoff. The pair table must have a verified protein-length-normalized `qtmscore`, `ttmscore` or `mintmscore` header; default `fident` and alignment-normalized `alntmscore` are rejected. The study uses `mintmscore` (min of both directions, i.e. normalized by the longer chain), so a short chain cannot join two complexes through a hit normalized by its own length (A11). |
| Entry resolution for the structure-quality gate | file value; when the file records none, the entry's resolution from curation metadata (SNAC `Resolution`, SAbDab `resolution`) by PDB ID, worst value of a multi-entry field; `resolution_source` recorded per row | Study-specific provenance rule; threshold unchanged | SNAC-DB distributes assembly-curated complex files without the deposition's resolution record [R34], so reading the file alone reports none for every `snac_db` entry and the required-resolution gate would reject the whole subset for a missing field rather than for quality. SAbDab publishes the entry's resolution in its summary table [R33], and SNAC's curation summaries carry the same field, so the value is the deposition's own, not an estimate. A field listing one value per entry is read at its worst (largest) value, because the gate is an upper bound. The metadata files actually used are listed with their SHA-256 in the audit report, and files under pipeline working directories are excluded, so the admission decision can be reproduced from declared inputs [R48] (PROTOCOL_AMENDMENTS.md A12). |
...
| Cross-partner graph edges | fixed KNN, k=3 | Study-specific preregistration | Chosen specifically to decouple input edge existence from the 4.5 Å primary label and the 3.5/5.0 Å sensitivity labels, preventing deterministic topology leakage. No paper is claimed to establish k=3 as optimal. |
...
**[R48]** Sandve, G. K., Nekrutenko, A., Taylor, J., & Hovig, E. (2013). Ten simple rules for reproducible computational research. *PLoS Computational Biology*, 9(10), e1003285. https://doi.org/10.1371/journal.pcbi.1003285
