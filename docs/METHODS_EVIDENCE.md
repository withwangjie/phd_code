...
| Feasibility-preserving XY mixer | Local one-hot registers remain inside the feasible Hamming-weight-one subspace | Direct methodological basis | Hadfield et al. [R28] generalize QAOA to alternating operators designed to mix only within a desired feasible subspace for hard-constrained optimization. The exact local all-pairs XY schedule used here is study-specific, but the feasibility-preserving mixer principle is directly literature-grounded. |
