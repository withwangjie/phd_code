...
import evaluate_complex_metrics as m
...
# already validated for evaluate_complex_dockq.py's offline tests).
...
class EvaluateComplexMetricsTests(unittest.TestCase):
    ...
    def test_invalid_arguments_rejected(self):
        ref = _make_complex()
        pred = copy.deepcopy(ref)
        with patch("evaluate_complex_metrics.read_structure_atoms", side_effect=lambda *a, **k: copy.deepcopy(ref)):
    ...
    def test_trajectory_multi_stage_and_deltas(self):
        ...
        with patch(
            "evaluate_complex_metrics.read_structure_atoms",
    ...
    def test_trajectory_error_names_failing_stage(self):
        ...
        with patch("evaluate_complex_metrics.read_structure_atoms", side_effect=[ref, broken]):
    ...
    def test_trajectory_csv_export_one_row_per_stage(self):
        ...
        with patch("evaluate_complex_metrics.read_structure_atoms", side_effect=[ref, ref, ref, ref]):
            trajectory = m.evaluate_trajectory(
                "ref.cif", stage_paths, receptor_chains=["A"], ligand_chains=["H"],
            )
        with tempfile.TemporaryDirectory() as tmp:
